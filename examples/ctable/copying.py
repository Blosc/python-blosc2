...
class Ride:
    ...
    waypoints: list[float] = b2.field(                        # list column (no default)
        b2.list(b2.float32(), nullable=True, batch_rows=16)   # noqa: RUF009
    )
...
waypoints = [
    None if rng.random() < 0.1 else rng.uniform(-90, 90, rng.integers(2, 8)).tolist()
    for _ in range(N)
]

data = list(zip(
    distances.tolist(), durations.tolist(),
    payments, waypoints, fares.tolist(), verified, strict=False,
))
