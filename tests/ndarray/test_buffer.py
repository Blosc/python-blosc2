...
@pytest.mark.parametrize("shape, chunks, blocks, dtype, urlpath, contiguous, meta",
                         [
                             ([450], [128], [25], "|S8", "frombuffer.b2nd", True, None),
                             ([20, 134, 13], [3, 13, 5], [3, 10, 5], np.complex128, "frombuffer.b2nd",
                              False, {"123": 123}),
...
def test_buffer(shape, chunks, blocks, dtype, urlpath, contiguous, meta):
    ...
    a = blosc2.frombuffer(buffer, shape, chunks=chunks, blocks=blocks,
