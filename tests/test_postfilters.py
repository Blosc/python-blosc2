...
@pytest.mark.parametrize("func, input_dtype, output_dtype",
                         [
                            ("postf1", np.dtype(np.int32), None),
                             ("postf1", np.dtype(np.int32), np.dtype(np.float32)),
                            ("postf3", np.dtype("M8[D]"), np.dtype(np.int64)),
                            ("postf2", np.dtype(np.float64), None),
                         ])
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"splitmode": blosc2.SplitMode.ALWAYS_SPLIT, "nthreads": 4}, {"schunk": None, "nthreads": 4}, 5, False, None),
        ({"codec": blosc2.Codec.LZ4HC}, {"nthreads": 4}, 3, False, "test_postfilters.b2frame"),
    ],
)
def test_postfilters(contiguous, urlpath, cparams, dparams, nchunks, func, input_dtype, output_dtype):
    ...
    if func == "postf1":
        ...
        def postf1(input, output):
            output[:] = input - 1
    elif func == "postf2":
        @blosc2.postfilter(schunk, input_dtype, output_dtype)
        def postf2(input, output):
            output[:] = input - np.pi
    else:
        @blosc2.postfilter(schunk, input_dtype, output_dtype)
        def postf3(input, output):
