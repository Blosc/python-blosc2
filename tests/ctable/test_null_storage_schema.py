...
def test_policy_defaults_to_sentinel():
    """Phase 2 ships the capability; the default flips a release later."""
    assert blosc2.NullPolicy().null_storage == "sentinel"


def test_policy_rejects_an_unknown_storage():
    with pytest.raises(ValueError, match="null_storage must be 'mask' or 'sentinel'"):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("bool_value", 255, blosc2.bool(nullable=True), bool),
...
def test_mask_skips_string_max_length_widening():
    ...
    sentinel_col = _resolved(blosc2.string(max_length=4, nullable=True), str)
...
def test_sentinel_default_is_unchanged():
    """The whole point of shipping opt-in first: nothing moves by default."""
    col = _resolved(blosc2.bool(nullable=True), bool)
