...
def test_delete_single_element():
    ...
    # Delete first
    t = CTable(Row, new_data=data, expected_size=50)
    t.delete(0)
    assert len(t) == 49
    assert not t._valid_rows[0]

    # Delete last
    t2 = CTable(Row, new_data=data, expected_size=50)
    t2.delete(-1)
    assert len(t2) == 49

    # Delete middle
    t3 = CTable(Row, new_data=data, expected_size=50)
    t3.delete(25)
    assert len(t3) == 49

    # Delete first 10 times in a row
    t4 = CTable(Row, new_data=data, expected_size=50)
    for i in range(10):
        t4.delete(0)
    ...
    t5 = CTable(Row, new_data=data, expected_size=50)
    for i in range(10):
        t5.delete(-1)
...
def test_delete_list_of_positions():
    ...
    t6 = CTable(Row, new_data=data, expected_size=50)
    t6.delete(list(range(0, 20, 2)))
    assert len(t6) == 40

    # First 10 rows
    t7 = CTable(Row, new_data=data, expected_size=50)
    t7.delete(list(range(0, 10)))
    assert len(t7) == 40

    # Last 10 rows
    t8 = CTable(Row, new_data=data, expected_size=50)
...
def test_delete_out_of_bounds():
    ...
    t2 = CTable(Row, new_data=generate_test_data(25), expected_size=50)
    assert len(t2) == 25
    with pytest.raises(IndexError):
        t2.delete(35)
    ...
    for pos in [0, 25, -1]:
        empty = CTable(Row, expected_size=50)
        assert len(empty) == 0
        with pytest.raises(IndexError):
            empty.delete(pos)
