...
if iobw:
    ...
    create_ZSTD_l5_8bits_disk = sizes_GB[: len(create_ZSTD_l5_8bits_disk)] / np.array(
        create_ZSTD_l5_8bits_disk
    )
    compute_ZSTD_l5_8bits_disk = sizes_GB[: len(compute_ZSTD_l5_8bits_disk)] / np.array(
        compute_ZSTD_l5_8bits_disk
    )
    create_ZSTD_l5_8bits_mem = sizes_GB[: len(create_ZSTD_l5_8bits_mem)] / np.array(create_ZSTD_l5_8bits_mem)
    compute_ZSTD_l5_8bits_mem = sizes_GB[: len(compute_ZSTD_l5_8bits_mem)] / np.array(
        compute_ZSTD_l5_8bits_mem
    )
    create_ZSTD_l5_12bits_disk = sizes_GB[: len(create_ZSTD_l5_12bits_disk)] / np.array(
        create_ZSTD_l5_12bits_disk
    )
    compute_ZSTD_l5_12bits_disk = sizes_GB[: len(compute_ZSTD_l5_12bits_disk)] / np.array(
        compute_ZSTD_l5_12bits_disk
    )
    create_ZSTD_l5_12bits_mem = sizes_GB[: len(create_ZSTD_l5_12bits_mem)] / np.array(
        create_ZSTD_l5_12bits_mem
    )
    compute_ZSTD_l5_12bits_mem = sizes_GB[: len(compute_ZSTD_l5_12bits_mem)] / np.array(
        compute_ZSTD_l5_12bits_mem
    )
    create_ZSTD_l5_16bits_disk = sizes_GB[: len(create_ZSTD_l5_16bits_disk)] / np.array(
        create_ZSTD_l5_16bits_disk
    )
    compute_ZSTD_l5_16bits_disk = sizes_GB[: len(compute_ZSTD_l5_16bits_disk)] / np.array(
        compute_ZSTD_l5_16bits_disk
    )
    create_ZSTD_l5_16bits_mem = sizes_GB[: len(create_ZSTD_l5_16bits_mem)] / np.array(
        create_ZSTD_l5_16bits_mem
    )
    compute_ZSTD_l5_16bits_mem = sizes_GB[: len(compute_ZSTD_l5_16bits_mem)] / np.array(
        compute_ZSTD_l5_16bits_mem
    )
    create_ZSTD_l5_24bits_disk = sizes_GB[: len(create_ZSTD_l5_24bits_disk)] / np.array(
        create_ZSTD_l5_24bits_disk
    )
    compute_ZSTD_l5_24bits_disk = sizes_GB[: len(compute_ZSTD_l5_24bits_disk)] / np.array(
        compute_ZSTD_l5_24bits_disk
    )
    create_ZSTD_l5_24bits_mem = sizes_GB[: len(create_ZSTD_l5_24bits_mem)] / np.array(
        create_ZSTD_l5_24bits_mem
    )
    compute_ZSTD_l5_24bits_mem = sizes_GB[: len(compute_ZSTD_l5_24bits_mem)] / np.array(
        compute_ZSTD_l5_24bits_mem
    )
    create_ZSTD_l5_32bits_disk = sizes_GB[: len(create_ZSTD_l5_32bits_disk)] / np.array(
        create_ZSTD_l5_32bits_disk
    )
    compute_ZSTD_l5_32bits_disk = sizes_GB[: len(compute_ZSTD_l5_32bits_disk)] / np.array(
        compute_ZSTD_l5_32bits_disk
    )
    create_ZSTD_l5_32bits_mem = sizes_GB[: len(create_ZSTD_l5_32bits_mem)] / np.array(
        create_ZSTD_l5_32bits_mem
    )
    compute_ZSTD_l5_32bits_mem = sizes_GB[: len(compute_ZSTD_l5_32bits_mem)] / np.array(
        compute_ZSTD_l5_32bits_mem
    )
...
def add_ram_limit(figure, compute=True):
    ...
    figure.add_annotation(
        x=np.log10(64), y=y1_max * 0.9, text="64 GB", showarrow=True, arrowhead=2, ax=40, ay=0, xref="x"
    )
...
fig_create.add_trace(
    go.Scatter(
        x=sizes_GB,
        y=create_ZSTD_l5_f32_disk,
        mode="lines+markers",
        name=labels["f32_disk"],
        line=dict(color="brown"),
    )
)
# fig_create.add_trace(go.Scatter(x=sizes_GB, y=create_ZSTD_l5_f32_mem, mode='lines+markers', name=labels["f32_mem"]))
fig_create.update_layout(
    title=f"Create operands: {title_}", xaxis_title="Size (GB)", yaxis_title=yaxis_title, xaxis_type="log"
)
...
fig_compute.update_layout(
    title=f"Blosc2 compute: {title_}", xaxis_title="Size (GB)", yaxis_title=yaxis_title, xaxis_type="log"
)
