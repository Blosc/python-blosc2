...
def run_benchmark(
    ...
    chunks, blocks = blosc2.compute_chunks_blocks(
        shapes[0], dtype=dtype, cparams=blosc2.CParams(codec=codec)
    )
    if aligned_chunks:
...
def create_combined_plot(
    ...
    for ax, axis in [(ax0, axes[0]), (ax1, axes[1])]:
        ...
        ax.set_title(
            f"Stack Performance for {num_arrays} arrays (axis={axis}) [{datadist}, {codec_str}]", fontsize=14
        )
        ax.set_xticks(x)
...
def main():
    ...
    for axis in [0, -1]:
        ...
        print(
            f"{'Size':<8} {'NumPy (GB/s)':<14} {'Unaligned (GB/s)':<18} "
        ...
        for size in sizes:
            # Run the benchmarks
            numpy_time, numpy_shape, data_size_gb = run_numpy_benchmark(
                num_arrays, size, axis=axis, dtype=dtype
            )
            unaligned_time, shape1, _ = run_benchmark(
                num_arrays,
                size,
                aligned_chunks=False,
                axis=axis,
            ...
            aligned_time, shape2, _ = run_benchmark(
                num_arrays, size, aligned_chunks=True, axis=axis, dtype=dtype, datadist=datadist, codec=codec
            )
            ...
            # Print results
            print(
                f"{size:<10} {numpy_speed:<14.2f} {unaligned_speed:<18.2f} {aligned_speed:<16.2f} "
            ...
            if any(shape != expected_shape for shape in shapes):
                for i, shape_name in enumerate(["NumPy", "Blosc2 unaligned", "Blosc2 aligned"]):
                    if shapes[i] != expected_shape:
                        print(
                            f"Warning: {shape_name} shape {shapes[i]} does not match expected {expected_shape}"
                        )
