...
def genarray(r, verbose=True):
    ...
    t = time.time()
    arr = blosc2.linspace(
        0, 1000, num=np.prod(shape), shape=shape, dtype=np.float64, urlpath=f"linspace{r}1D.b2nd", mode="w"
    )
    t = time.time() - t
...
target_sizes = np.float64(np.array([0.1, 0.2, 0.5, 1, 2, 3]))
...
for d in target_sizes:
    ...
    idx = (
        rng.integers(low=0, high=arr.shape[0], size=(1000,))
        if SPARSE
        else rng.integers(low=0, high=arr.shape[0], size=(arr.shape[0] // 4,))
    )
