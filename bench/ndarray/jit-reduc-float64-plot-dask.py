...
def add_ram_limit(figure, compute=True):
    ...
    if amd:
        ...
        figure.add_annotation(
            x=64, y=y1_max * 0.9, text="64 GB RAM", showarrow=True, arrowhead=2, ax=45, ay=0
        )
...
fig_create.add_trace(go.Scatter(x=sizes_GB, y=create_l0, mode="lines+markers", name=labels["l0"]))
fig_create.add_trace(go.Scatter(x=sizes_GB, y=create_l0_dask, mode="lines+markers", name=labels["l0_dask"]))
fig_create.add_trace(go.Scatter(x=sizes_GB, y=create_LZ4_l1, mode="lines+markers", name=labels["LZ4_l1"]))
fig_create.add_trace(
    go.Scatter(x=sizes_GB, y=create_LZ4_l1_dask, mode="lines+markers", name=labels["LZ4_l1_dask"])
)
fig_create.add_trace(go.Scatter(x=sizes_GB, y=create_ZSTD_l1, mode="lines+markers", name=labels["ZSTD_l1"]))
fig_create.add_trace(
...
fig_create.add_trace(
    go.Scatter(
        x=sizes_GB,
        y=create_numpy_numba,
        mode="lines+markers",
        name=labels["numpy_numba"],
        line=dict(color="black", dash="dot"),
    )
)
fig_create.add_trace(
    go.Scatter(
        x=sizes_GB, y=create_numpy_jit, mode="lines+markers", name=labels["numpy"], line=dict(color="brown")
    )
)
fig_create.add_trace(
    go.Scatter(
        ...
        y=create_numpy_jit,
        mode="lines+markers",
        name=labels["numpy_dask"],
        line=dict(color="cyan"),
    )
)
fig_create.update_layout(
    title=f"Create operands: {title_}", xaxis_title="Size (GB)", yaxis_title=yaxis_title
)
...
y1_max = max(
    ...
    max(compute_l0_disk),
    max(compute_LZ4_l1),
    max(compute_LZ4_l1_disk),
    max(compute_ZSTD_l1),
    max(compute_ZSTD_l1_disk),
    max(compute_numpy),
    max(compute_numpy_jit),
...
fig_compute.add_trace(
    go.Scatter(
        x=sizes_GB,
        y=compute_LZ4_l1_dask,
        mode="lines+markers",
        name=labels["LZ4_l1_dask"],
...
fig_compute.update_layout(
    title=f"Blosc2 vs others; compute: {title_}", xaxis_title="Size (GB)", yaxis_title=yaxis_title
)
