    ...
    for xp in [blosc2, np, torch]:
        ...
        for n in shapes:
            ...
            for lib in [blosc2, torch, np]:
                # Generate matrices
                matrix_a = lib.full(shape_a, fill_value=3.0, dtype=lib.float32)
                matrix_b = lib.full(shape_b, fill_value=2.4, dtype=lib.float32)
                matrix_c = lib.full(shape_b[:1], fill_value=0.4, dtype=lib.float32)
                _time = 0
                # multiplication
                if (
                    (xp.__name__ == "torch" and lib.__name__ == "torch")
                ...
                print(
                    f"Size = {np.round(size_gb, 1)} GB, {xp.__name__.upper()}_{lib.__name__} Performance = {_time:.2f} s"
                )
        ...
        with open(f"{filename}_{xp.__name__.upper()}.pkl", "wb") as f:
            pickle.dump(
                {
                    "blosc2": {"Matrix Size (GB)": sizes, "Mean Time (s)": mean_times["blosc2"]},
                    "numpy": {"Matrix Size (GB)": sizes, "Mean Time (s)": mean_times["numpy"]},
                    "torch": {"Matrix Size (GB)": sizes, "Mean Time (s)": mean_times["torch"]},
                },
                f,
            )
