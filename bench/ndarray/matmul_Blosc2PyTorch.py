...
if CREATE:
    ...
    def build_dense_rowwarp_matrix(
        out_h=2000,
        in_h=2167,
        scale=1.0,
    ...
    out = blosc2.empty(
        shape=(NUM_IMAGES, OUT_H, IN_H), dtype=dtype, urlpath="transform.b2nd", mode="w", cparams=cparams
    )
    ...
    with h5py.File(fname_in, "r") as fr:  # load file and process to blosc2 array
        ...
        b2im = blosc2.empty(
            shape=(2 * len(dset), 2167, 2070), dtype=dtype, cparams=cparams, urlpath="kevlar.b2nd", mode="w"
        )
    ...
    with h5py.File(fname_out, "w") as fw:
        b2comp = hdf5plugin.Blosc2(
            cname="lz4", clevel=1, filters=hdf5plugin.Blosc2.SHUFFLE
        )  # just for identification, no compression algorithm specified
        ...
        for i in tqdm(
            range(0, len(b2im), batch_size), desc="Converting transform and data matrices to HDF5"
        ):
...
print(
    f"Total working set size: {round((np.prod(dset_a.shape) / 2**30 + np.prod(dset_a.shape[:-1] + dset_b.shape[-1:]) / 2**30 + np.prod(dset_b.shape) / 2**30) * dset_b.dtype.itemsize, 1)} GB."
)
...
results = {
    "blosc_chunks_out": chunks_blosc_out,
    "blosc_chunks": chunks_blosc,
    "hdf5_chunks_out": hdf5_chunks_out,
    "hdf5_chunks": hdf5_chunks,
    "ABshape": in_shapes,
    "dtype": out_blosc.dtype,
    "PyTorch": torch_time,
    "Blosc2": blosc_time,
}
fname = "matmul_OOC"
