...
def add_ram_limit(figure, compute=True):
    ...
    if amd:
        ...
        figure.add_annotation(x=24, y=y1_max * 0.9, text="24 GB", showarrow=True, arrowhead=2, ax=40, ay=0)
    elif intel:
        ...
        figure.add_annotation(x=32, y=y1_max * 0.9, text="32 GB", showarrow=True, arrowhead=2, ax=40, ay=0)
    else:
        # y1_max = 35 if compute else y1_max
        figure.add_shape(
            type="line",
...
labels = dict(
    l0="No compression",
    BLOSCLZ_l7="BLOSCLZ lvl=7",
    LZ4_l1="LZ4 lvl=1",
    ZSTD_l1="ZSTD lvl=1",
    numpy="NumPy engine",
    numpy_jit="NumPy with @blosc2.jit",
)
...
if plotly:
    ...
    fig_create.add_trace(
        go.Scatter(
            x=sizes_GB, y=create_ZSTD_l1_disk, mode="lines+markers", name=labels["ZSTD_l1"] + " (disk)"
        )
    )
    fig_create.add_trace(
        go.Scatter(
    ...
    fig_compute.add_trace(
        go.Scatter(
            x=sizes_GB, y=compute_LZ4_l1_disk, mode="lines+markers", name=labels["LZ4_l1"] + " (disk)"
        )
    )
    fig_compute.add_trace(
    ...
    fig_compute.add_trace(
        go.Scatter(
            x=sizes_GB, y=compute_ZSTD_l1_disk, mode="lines+markers", name=labels["ZSTD_l1"] + " (disk)"
        )
    )
    fig_compute.add_trace(
        go.Scatter(
    ...
    fig_compute.update_layout(
        title=f"Blosc2 compute: {title_}", xaxis_title="Size (GB)", yaxis_title=yaxis_title
    )
