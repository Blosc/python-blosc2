...
labels = [
    "NumPy",
    "blosc2.jit (np.ndarray)",
    "blosc2.jit (blosc2.NDArray)",
    "numba.jit (first run)",
    "numba.jit (cached)",
]
...
colors = [
    ...
    "#99CC99",
    "#99CC99",
]  # Red for NumPy, Blue for blosc2, Green for numba
...
for i, (bar, time) in enumerate(zip(bars, times_rev)):
    ...
    if i < len(times_rev) - 1:  # Skip the last bar (NumPy)
        ...
        ax.annotate(
            f"({speedup:.1f}x)", (bar.get_width() + 0.05, bar.get_y() + bar.get_height() / 2), va="center"
        )
