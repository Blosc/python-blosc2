...
sizes = np.sqrt(
    1024**3 * np.array([1 / 2**5, 1 / 2**4, 1 / 2**3, 1 / 2**2, 1 / 2, 1]) / dtype.itemsize
)  # operand size up to 1GB
...
def myudf_numba(inputs, output, offset):
    a, b, c = inputs
    output[:] = np.exp(
        np.sqrt((np.sin(a) ** 2 + (np.cos(b) + np.arctan(c)) ** 3) * (1 + np.sin(b) ** 2 + np.cos(c) ** 2))
    )
...
if nthreads == MAX_THREADS:
    plt.loglog(gigas, blosc2_times, color="b", ls="-", label="Blosc2", lw=3)
boost = np.mean(np.divide(blosc2_parallel_times, blosc2_times))
plt.loglog(gigas, blosc2_parallel_times, color=c_, ls="-", label="Blosc2 + Numba", lw=3)

plt.xlabel("Working set size (GB)")
plt.ylabel("GFLOPS / s")
plt.xticks([0.1, 0.5, 1, 2, 4], [0.1, 0.5, 1, 2, 4])
...
plt.annotate(
    f"Performance boost: {round(boost, 1)}x !",
    (0.31, 0.6),
    xycoords="figure fraction",
    bbox=dict(boxstyle="round", fc="0.8", color="b", alpha=0.5),
)
idx = len(gigas) // 4
plt.annotate(
    "",
    xytext=(gigas[idx], blosc2_times[idx]),
    xy=(gigas[idx], blosc2_parallel_times[idx]),
