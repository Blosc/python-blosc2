...
def add_ram_limit(figure, compute=True):
    ...
    figure.add_annotation(
        x=x64, y=y1_max * 0.9, text="64 GB RAM", showarrow=True, arrowhead=2, ax=45, ay=0, xref="x"
    )
...
fig_create.update_layout(
    title=f"Create operands {alt_tit}: {title_}",
    xaxis_title="Size (GB)",
...
if xaxis_type == "log":
    fig_compute.add_trace(
        go.Scatter(
            x=sizes_GB, y=compute_numpy, mode="lines+markers", name=labels["numpy"], line=dict(color="brown")
        )
    )
...
fig_compute.update_layout(
    title=f"Blosc2 compute {alt_tit}: {title_}",
    xaxis_title="Size (GB)",
    yaxis_title=yaxis_title,
    xaxis_type=xaxis_type,
)
