...
print("\nTimes for compressing/decompressing with clevel=%d and %d threads" % (
    clevel, blosc2.nthreads))
for (in_, label) in arrays:
    print("\n*** %s ***" % label)
    for cname in blosc2.compressor_list():
        for filter in [blosc2.NOFILTER, blosc2.SHUFFLE, blosc2.BITSHUFFLE, blosc2.DELTA, blosc2.TRUNC_PREC]:
            t0 = time.time()
            ...
            print("  *** %-8s, %-10s *** %6.3f s (%.2f GB/s) / %5.3f s (%.2f GB/s)" % (
                cname, filter, tc, ((N*8 / tc) / 2**30), td, ((N*8 / td) / 2**30)), end='')
