...
def b2_native(urlpath=None):
    t0 = time()
    a = blosc2.linspace(0.0, 1.0, N, urlpath=urlpath, mode="w")
    ...
    t1 = time()
    print(
        f"Time to create a linspace array: {t1 - t0:.2f}s, bandwidth: {a.nbytes / (t1 - t0) / 1e9:.2f} GB/s"
    )
    # print(a.info)
...
def b2_b2zip(urlpath):
    ...
    with blosc2.TreeStore(localpath=urlpath, mode="w") as tstore:
        a = blosc2.linspace(0.0, 1.0, N)
        # a = blosc2.linspace(0., 1., 2**27, cparams=blosc2.CParams(codec=blosc2.Codec.LZ4))
        tstore["/b"] = a
    t1 = time()
    print(
        f"Time to store a linspace array: {t1 - t0:.2f}s, bandwidth: {a.nbytes / (t1 - t0) / 1e9:.2f} GB/s"
    )
