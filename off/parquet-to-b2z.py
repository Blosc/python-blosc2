...
"""Import/export Parquet datasets through a CTable .b2z.

Default mode imports parquet -> .b2z using CTable.from_arrow_batches().
Additional modes:

* --export: export an existing .b2z -> parquet.
* --roundtrip: import parquet -> .b2z -> parquet and assess differences.
...
def require_pyarrow():
    try:
        ...
        raise ImportError(
            "import-to-b2z-gpt.py requires pyarrow; install it with: pip install pyarrow"
        ) from exc
    return pa, pc, pq
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Import/export Parquet datasets via CTable (.b2z).",
...
def scan_string_max_lengths(pa, pc, pq, input_path: Path, args) -> dict[str, int]:
    ...
    rows_seen = 0
    for batch in pf.iter_batches(batch_size=args.batch_size, columns=names):
        for name in names:
        ...
        rows_seen += len(batch)
        if args.string_scan == "sample" and rows_seen >= args.sample_rows:
            break

    result = {}
    for name, observed in lengths.items():
        estimated = int(observed * args.string_slack) if args.string_scan == "sample" else observed
...
def classify_columns(
    ...
    for field in schema:
        ...
        if pa.types.is_string(t) or pa.types.is_large_string(t):
            ...
            if field.name in force_list_strings or max_len > string_fixed_threshold:
...
def import_once(args, input_path: Path, output_path: Path, force_list_strings: set[str]):
    ...
    print(
        ...
        f" ({args.string_scan}{', rows=' + format(args.sample_rows, ',') if args.string_scan == 'sample' else ''})…"
    )
    ...
    fixed_cols, list_wrap_cols, conversions, nullable_scalars = classify_columns(
        pa, parquet_schema, string_max_lengths, args.string_fixed_threshold, force_list_strings
    )
...
def import_parquet_to_b2z(args, input_path: Path, output_path: Path):
    ...
    if args.output_path is not None and output_path.suffix != ".b2z":
        raise ValueError("output_path must use the .b2z extension")
    if not input_path.exists():
        raise FileNotFoundError(f"Input file not found: {input_path}")
    prepare_output(output_path, args.overwrite)

    force_list_strings: set[str] = set()
...
def export_b2z_to_parquet(input_path: Path, output_path: Path, *, batch_size: int, overwrite: bool):
...
def main() -> None:
    ...
    if args.export:
        ...
        export_b2z_to_parquet(input_path, output_path, batch_size=args.batch_size, overwrite=args.overwrite)
        return
    if args.roundtrip:
        input_path = args.input_path
        b2z_path = args.output_path or DEFAULT_B2Z
        roundtrip_path = DEFAULT_ROUNDTRIP_PARQUET
        selected = import_parquet_to_b2z(args, input_path, b2z_path)
        exported = export_b2z_to_parquet(
    ...
    import_parquet_to_b2z(args, args.input_path, output_path)
